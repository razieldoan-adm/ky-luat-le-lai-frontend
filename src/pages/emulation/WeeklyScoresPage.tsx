...
export default function WeeklyScoresPage() {
  ...
  const [changed, setChanged] = useState(false); // bổ sung flag thay đổi
  ...
  const recalcAndRank = (input: ScoreRow[]): ScoreRow[] => {
    let updated = input.map((s) => {
  ...
  const handleCalculate = () => {
    ...
    const updated = recalcAndRank(scores);
    setScores(updated);
    setCalculated(true);
    setSaved(false);
    setChanged(true);
