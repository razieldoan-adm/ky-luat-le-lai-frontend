...
const WeeklyScoresPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const res = await api.get(`/api/class-weekly-scores?weekNumber=${selectedWeek}`);
      setScores(res.data);
  ...
  // hàm tính lại totalScore (giữ nguyên)
  ...
  // chỉnh sửa điểm trực tiếp trên form
  const handleScoreChange = (className: string, field: string, value: number) => {
  ...
  const handleLoadData = async () => {
    const res = await api.get(`/api/class-weekly-scores/temp?weekNumber=${selectedWeek}`);
    setScores(res.data);
    setOriginalScores(res.data);
    setIsRanked(false);
    setIsSaved(false);
  ...
  // Cập nhật (tính lại rank từ dữ liệu gốc)
            ...
            <TableRow key={row.className}>
              ...
              <TableCell>
                <TextField
                  ...
                  onChange={(e) =>
                    handleScoreChange(row.className, "academicScore", Number(e.target.value))
                  }
                />
              </TableCell>
              <TableCell>
                <TextField
                  ...
                  onChange={(e) =>
                    handleScoreChange(row.className, "bonusScore", Number(e.target.value))
                  }
                />
              </TableCell>
