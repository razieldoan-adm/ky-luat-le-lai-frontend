...
export default function WeeklyScoresPage() {
  ...
  const handleLoadData = async () => {
    ...
    try {
      ...
      const data: ScoreRow[] = classes.map((cls) => {
        const v = violations.data.find((x: any) => x.className === cls.className)?.totalScore || 0;
        const h = hygiene.data.find((x: any) => x.className === cls.className)?.totalScore || 0;
        const a = attendance.data.find((x: any) => x.className === cls.className)?.totalScore || 0;
        const l = lineup.data.find((x: any) => x.className === cls.className)?.totalScore || 0;
  ...
  // Gom nhóm theo khối để render bảng tách riêng
